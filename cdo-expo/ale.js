      ...
      ;(function () {
        ...
        function apiRequest(endpoint, method, payload) {
          ...
          if (method.toUpperCase() === "GET") {
            ...
            xhr.open("GET", "/datablock_storage/" + channel_id + "/" + endpoint + "?" + query, false);
            xhr.send();
          } else {
            xhr.open(method, "/datablock_storage/" + channel_id + "/" + endpoint, false);
            xhr.setRequestHeader("Content-Type", "application/json");
            xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
            xhr.setRequestHeader("X-CSRF-Token", localStorage.userId);
            xhr.withCredentials = true;
            xhr.send(JSON.stringify(payload));
          }
          return result;
        }
        function onSuccess(callback, value) {
        ...
        Object.defineProperties(window, {
          setKeyValueSync: {
            value: function (key, value, success, failure) {
              const handleSuccess = onSuccess.bind(null, success);
              const handleFailure = onError.bind(null, failure);
              try {
                if (typeof key !== "string") throw "invalid param: key unreachable";
                const result = apiRequest("set_key_value", "POST", {
                  key: key,
          ...
          getKeyValueSync: {
            value: function (key, success, failure) {
              const handleSuccess = onSuccess.bind(null, success);
              const handleFailure = onError.bind(null, failure);
              try {
                if (typeof key !== "string") throw "invalid param: key unreachable";
                const result = apiRequest("get_key_value", "GET", { key: key });
                handleSuccess(result);
              } catch (err) {
                handleFailure(err);
              }
          ...
          getColumn: {
            value: function (tableName, columnName, success, failure) {
              const handleSuccess = onSuccess.bind(null, success);
              const handleFailure = onError.bind(null, failure);
              try {
                if (typeof tableName !== "string" || typeof columnName !== "string") throw "invalid param(s): table or column is not a string";
                const column = apiRequest("get_column", "GET", {
                  table_name: tableName,
                  column_name: columnName
                });
                if (column === undefined) throw "table or column has not been added to the dataset";
                handleSuccess(column);
                return column;
              } catch (err) {
                handleFailure(err);
              }
          ...
          createRecordSync: {
            value: function (tableName, record, success, failure) {
              const handleSuccess = onSuccess.bind(null, success);
              const handleFailure = onError.bind(null, failure);
              try {
                if (typeof tableName !== "string" || typeof record !== "object" || record === null) throw "invalid param(s): table <string> or record <object> is an invalid type";
                const result = apiRequest("create_record", "POST", {
                  table_name: tableName,
          ...
          readRecordsSync: {
            value: function (tableName, success, failure) {
              const handleSuccess = onSuccess.bind(null, success);
              const handleFailure = onError.bind(null, failure);
              try {
                if (typeof tableName !== "string") throw "invalid param: table <string> is invalid";
                const result = apiRequest("read_records", "GET", {
                  table_name: tableName,
          ...
          updateRecordSync: {
            value: function (tableName, record, success, failure) {
              ...
              const handleFailure = onError.bind(null, failure);
              try {
                if (typeof tableName !== "string" || typeof record !== "object" || record === null) throw "invalid param(s): table <string>, record <object>";
                if (typeof record.id !== "number" && record.id > -1 && record.id < Infinity) throw "invalid record entry id";
                const result = apiRequest("update_record", "PUT", {
                  table_name: tableName,
                  record_id: record.id,
                  record_json: JSON.stringify(record)
                });
          ...
          deleteRecordSync: {
            value: function (tableName, record, success, failure) {
              ...
              try {
                if (typeof tableName !== "string" || typeof record !== "object" || record === null) throw "invalid param(s): table <string>, record <object>";
                if (typeof record.id !== "number" && record.id > -1 && record.id < Infinity) throw "invalid record entry id";
                const result = apiRequest("delete_record", "DELETE", {
                  table_name: tableName,
                  record_id: record.id
                });
                handleSuccess(result);
          ...
          startWebRequestSync: {
            value: function (url, success, failure) {
              const xhr = new XMLHttpRequest();
              const handleSuccess = onSuccess.bind(null, success);
              const handleFailure = onError.bind(null, failure);
              xhr.onreadystatechange = function () {
                if (this.readyState === 4) {
                  if (xhr.status > 199 && xhr.status < 400) {
                    ...
                    handleSuccess("Request Responded With: " + xhr.status + " , Is not a safe range");
                  }
                }
              };
              xhr.onerror = function () {
