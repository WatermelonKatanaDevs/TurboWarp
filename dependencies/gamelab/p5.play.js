...
;(function (root, factory) {
    ...
    function defineLazyP5Property(name, getter) {
        Object.defineProperty(p5.prototype, name, {
            ...
            get: function () {
                var context =
                    this instanceof p5 && !this._isGlobal ? this : window
    ...
    p5.prototype.imageElement = function (
        ...
        if (this._renderer._tint) {
            ...
            if (
                this._tempCanvas.width < vals.w ||
                this._tempCanvas.height < vals.h
            ) {
                this._tempCanvas.width = Math.max(
                    this._tempCanvas.width,
                    vals.w
                )
                this._tempCanvas.height = Math.max(
                    this._tempCanvas.height,
                    vals.h
                )
            } else {
    ...
    p5.prototype.mouseIsOver = function (sprite) {
        ...
        if (this.camera.active) {
            mousePosition = this.createVector(
                this.camera.mouseX,
                this.camera.mouseY
            )
        } else {
        ...
        return sprite.collider.overlap(
            new window.p5.PointCollider(mousePosition)
        )
    }

    p5.prototype.mousePressedOver = function (sprite) {
        return (
            (this.mouseIsPressed || this.touchIsDown) &&
            this.mouseIsOver(sprite)
        )
    }
   ...
   */
        ...
        this.mouseUpdate = function () {
            ...
            if (this.collider) {
                ...
                else mousePosition = createVector(p5Inst.mouseX, p5Inst.mouseY)
                ...
                if (
                    this.mouseIsOver &&
                    (pInst.mouseIsPressed || pInst.touchIsDown)
                )
        ...
        this.setCollider = function (
            ...
            if (_type === 'point') {
                ...
                this.collider = p5.CircleCollider.createFromSprite(
                    this,
                    offset,
                    width
                )
                ...
                this.collider =
                    p5.AxisAlignedBoundingBoxCollider.createFromSprite(
                        this,
                        offset,
                        width,
                        height
        ...
        this.addImage = function () {
            if (
                ...
                arguments[1] instanceof p5.Image
            )
        ...
        this.addAnimation = function (label) {
            ...
            if (typeof label !== 'string') {
                ...
                this._internalWidth =
                    newAnimation.getWidth() * abs(this._getScaleX())
                this._internalHeight =
        ...
        this._collideWith = function (type, target, callback) {
            ...
            for (var i = 0; i < others.length; i++) {
                result =
                    this._collideWithOne(type, others[i], callback) || result
            }
            return result
        }
        ...
        this._findDisplacement = function (target) {
            ...
            if (this._doSweptCollidersOverlap(target)) {
                ...
                var timestep = Math.max(
                    0.015,
                    radiusOnVelocityAxis / relativeVelocity
                )
                ...
                if (timestep < 1) {
                    ...
                    for (var i = timestep; i < 1; i += timestep) {
                        ...
                        var displacement = this.collider.collide(
                            target.collider
                        )
                        if (displacement.x !== 0 || displacement.y !== 0) {
    ...
    function Group() {
        ...
        function _groupCollide(type, target, callback) {
            ...
            for (var i = 0; i < this.size(); i++)
                didCollide =
                    this.get(i)._collideWith(type, target, callback) ||
                    didCollide
            return didCollide
        }
        ...
        array.setAnimationEach = array.callMethodEach.bind(
            array,
            'setAnimation'
        )
    ...
    p5.prototype.createEdgeSprites = function () {
        ...
        context.leftEdge = context.createSprite(
            -edgeThickness / 2,
            height / 2,
            edgeThickness,
            height
        )
        context.rightEdge = context.createSprite(
            width + edgeThickness / 2,
            height / 2,
            edgeThickness,
            height
        )
        context.topEdge = context.createSprite(
            width / 2,
            -edgeThickness / 2,
            width,
            edgeThickness
        )
        context.bottomEdge = context.createSprite(
            width / 2,
    ...
    function Animation(pInst) {
        ...
        // Sprite sheet mode
            ...
            for (i = 0; i < frameArguments.length; i++) {
                ...
                else this.images.push(p5Inst.loadImage(frameArguments[i]))
            }
        }
        ...
        this.draw = function (x, y, r) {
            ...
            if (this.visible) {
                ...
                if (frame_info) {
                    var missingX =
                        (frame_info.sourceW || frame_info.width) -
                        frame_info.width
                    var missingY =
                        (frame_info.sourceH || frame_info.height) -
                        frame_info.height
    ...
    function SpriteSheet(pInst) {
        ...
        var longArgs =
            spriteSheetArgs.length === 4 || spriteSheetArgs.length === 5
        ...
        if (typeof this.nodes[0] !== 'undefined') {
            ...
            if (index !== -1) {
                returnObjects = returnObjects.concat(
                    this.nodes[index].retrieve(pRect)
                )
    ...
    p5.CollisionShape.prototype.collide = function (other) {
        ...
        for (var i = 0; i < candidateAxes.length; i++) {
            ...
            if (overlap <= 0) {
                ...
                if (
                    deltaOfCentersOnAxis.x === 0 &&
                    deltaOfCentersOnAxis.y === 0
                ) {
     ...
     * @protected
     */
    p5.AxisAlignedBoundingBoxCollider.prototype._onTransformChanged =
        function () {
            p5.CollisionShape.prototype._onTransformChanged.call(this)
            this._computeHalfDiagonals()
        }
     ...
     * @return {Array.<p5.Vector>}
     */
    p5.AxisAlignedBoundingBoxCollider.prototype._computeHalfDiagonals =
        function () {
     ...
     * @return {Array.<p5.Vector>}
     */
    p5.AxisAlignedBoundingBoxCollider.prototype._getCandidateAxes =
        function () {
     ...
     */
    p5.OrientedBoundingBoxCollider = function (
        center,
        width,
        height,
        rotation
     ...
     * @param {Sprite} sprite
     */
    p5.OrientedBoundingBoxCollider.prototype.updateSweptColliderFromSprite =
        function (sprite) {
            var vMagnitude = sprite.velocity.mag()
            var vPerpendicular = new p5.Vector(
                sprite.velocity.y,
                -sprite.velocity.x
            )
            this._width =
                vMagnitude +
                2 * sprite.collider._getRadiusOnAxis(sprite.velocity)
